...
def load_tabular_records(path: Path | str, *, limit: int | None = None) -> list[dict[str, Any]]:
    ...
    if suffix == ".json":
        ...
        if not isinstance(value, list):
            raise ValueError(f"{path}: expected a JSON list or object containing records/items")
...
class WikiLeaksPlusDAdapter:
    ...
    def normalize(self, record: dict[str, Any]) -> SourceItem:
        ...
        source_url = _text(_lookup(record, "source_url", "url")) or (
            f"https://wikileaks.org/plusd/cables/{reference}.html"
        )
        ...
        return SourceItem(
            ...
            title_raw=_text(_lookup(record, "subject", "title", "caption")),
            description_raw=_text(_lookup(record, "body", "content", "text", "cable", "document", "summary")),
            creator_raw=_text(_lookup(record, "origin", "from", "sender", "office")),
            date_raw=_text(_lookup(record, "date", "document_date", "datetime", "sent_at")),
            ...
            metadata_raw={
                **metadata,
                "destinations": _split_multi(
                    _lookup(record, "destinations", "destination", "to", "recipients")
                ),
                ...
                "tags": _split_multi(
                    _lookup(
                        record,
                        "tags",
                        "traffic_analysis_tags",
                        "traffic analysis by geography and subject",
                    )
                ),
                "references_to": _split_multi(
                    _lookup(record, "references_to", "references", "ref", "refs")
                ),
...
class WikiLeaksWarDiariesAdapter:
    ...
    def normalize(self, record: dict[str, Any]) -> SourceItem:
        ...
        source_url = _text(_lookup(record, "source_url", "url")) or (
            f"https://warlogs.wikileaks.org/id/{public_id}/"
        )
        mgrs = _text(_lookup(record, "mgrs", "mgrs coordinate", "mgrs_coordinate"))
        explicit_location = _text(
            _lookup(record, "location", "named_location", "event_location", "place")
        )
        metadata = dict(record)
        ...
        return SourceItem(
            ...
            collection_raw=_text(_lookup(record, "release", "collection", "dataset"))
            or "Iraq/Afghan War Diaries",
